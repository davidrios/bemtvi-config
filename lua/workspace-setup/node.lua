...
-- async seams — `btv.fs` for the directory, `btv.run` for npm — so nothing
...
--
-- The packages go under `<workspace>/.bemtvi/lsp` (the old `.neovim/lsp`), so a
...
-- that never fills in).
M.init = btv.async(function()
  local lsp_dir = M.lsp_dir()
  btv.await(btv.fs.mkdir(lsp_dir, { recursive = true }))
  ...
  -- to pick up. Scoped to `lsp/` on purpose — `.bemtvi/config.json` one level up
  -- is meant to be committed. Only written when absent, so an edit here stands.
  local gitignore = lsp_dir .. "/.gitignore"
  if not btv.await(btv.fs.exists(gitignore)) then
    btv.await(btv.fs.write(gitignore, "*\n"))
  end

  if not btv.await(btv.fs.exists(lsp_dir .. "/package.json")) then
    local res = btv.await(btv.run({ cmd = "npm", args = { "init", "-y" }, cwd = lsp_dir }))
    if res.code ~= 0 then
...
M.install_packages = btv.async(function(lsp_dir, packages)
  btv.notify("installing node packages...", btv.log.levels.INFO)
  ...
  local res = btv.await(btv.run({ cmd = "npm", args = args, cwd = lsp_dir }))
  if res.code ~= 0 then
    error("workspace-setup.node: error adding node packages: " .. res.stderr, 0)
  end

  btv.notify("node packages installed", btv.log.levels.INFO)
  return true
end)

-- init + install in one step, resolving the `node_modules` dir the server
-- commands are built from.
M.ensure = btv.async(function(packages)
  local lsp_dir = btv.await(M.init())
  btv.await(M.install_packages(lsp_dir, packages))
